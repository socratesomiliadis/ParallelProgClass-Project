...
func main() {
	...
	for {
		...
		go handleConnection(conn, PIsStore)
	}
}

func handleConnection(conn net.Conn, PIsStore map[int]float64) {
	...
	outmsg := processRequest(req, PIsStore)
...
func processRequest(req Request, PIsStore map[int]float64) Response {
	...
	storedPi, ok := PIsStore[numSteps]

	if ok {
		res = Response{Message: fmt.Sprintf("(hit) Pi is approximately %.16f", storedPi)}
	} else {
		...
		PIsStore[numSteps] = pi

		res = Response{Message: fmt.Sprintf("(miss) Pi is approximately %.16f", pi)}
	}
